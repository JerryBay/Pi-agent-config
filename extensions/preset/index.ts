 ...
 *
 * Config files (merged, later entries take precedence):
 * - <package>/presets.json (portable defaults)
...
function readPresets(path: string, label: string): PresetsConfig {
	if (!existsSync(path)) return {};
	try {
		return JSON.parse(readFileSync(path, "utf-8"));
	} catch (err) {
		console.error(`Failed to load ${label} presets from ${path}: ${err}`);
		return {};
	}
}

/**
 * Load bundled, global, and project-local presets in ascending precedence.
 */
function loadPresets(cwd: string): PresetsConfig {
	const bundledPath = fileURLToPath(new URL("../../presets.json", import.meta.url));
	const globalPath = join(getAgentDir(), "presets.json");
	const projectPath = join(cwd, CONFIG_DIR_NAME, "presets.json");

	const bundledPresets = readPresets(bundledPath, "bundled");
	const globalPresets = bundledPath === globalPath ? {} : readPresets(globalPath, "global");
	const projectPresets = readPresets(projectPath, "project-local");

	return { ...bundledPresets, ...globalPresets, ...projectPresets };
}
